...
class MoviesAdapter(private val moviesWithGenres: List<MoviesWithGenres>,
                    val callback: (MoviesWithGenres) -> Unit) : RecyclerView.Adapter<MoviesAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) =
        with(moviesWithGenres[position]) {
            val posterURL = EXTRA_IMG_URL_W500 + this.movie.posterPath
            Picasso.get().load(posterURL).into(holder.binding.ivMovie)
            holder.binding.textItemScore.text = this.movie.voteAverage.toString()
